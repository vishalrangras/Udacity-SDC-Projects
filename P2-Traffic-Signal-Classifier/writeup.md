...
The images are having different sizes, angle orientation and luminious intensity which should be the good enough candidate for the model to try and predict based on its learning. After resizing and pre-processing the internet images, the internet images appears as below:
...
The model was able to correctly guess 7 of the 10 traffic signs, which gives an accuracy of 70%. This accuracy is less compared to the test set accuracy of 0.885.
...
Here are the values of probabilities and image id for each image:
